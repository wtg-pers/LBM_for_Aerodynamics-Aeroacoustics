...
| 항목 | 결정 | 근거 |
|---|---|---|
| 분할 축 | **y-축 1D 분할, 전 레벨 공통** | ①x-분할은 우리 nested 토폴로지에 치명적 불균형(L4=슬랩 x-두께 57셀뿐, 로터 랭크가 일 독식) ②y/z는 전 레벨 extent가 큼(L4 681) → 레벨별 균등분할 ③1D가 halo 면 2개=최소 |
...
| MLG 분할 | **레벨별 독립 y-분할**(각 레벨 자신의 Ny를 N등분) + 레벨별 halo(코스=2: C2F cubic stencil, 파인=2: 안전) | blueprint §3.1/3.2. 균형: 각 레벨 updates가 균등 분할(L4=총 58%도 균등) |
...
- **M1 — 인프라+단일격자**: `src/parallel/`(Partition[y], HaloBandExchanger[v1], MPI runner 배선),
  esoteric 단일레벨 주기박스 2-rank. **게이트 G-M1**: 2-rank vs 1-rank rho/u bit(3090)·질량보존.
  (mpi4py 로컬 미설치 시: 통신층에 mock-loopback 모드 넣어 로컬 2-"rank" 검증 + 클러스터 실검증 이원화.)
- **M2 — MLG**: 레벨별 y-분할(OverlapRegion/coupling 슬라이스의 랭크-로컬화), 경계정렬 규칙. 
...
- (b) MLG 경계정렬: 레벨별 Ny가 2^k 배수 관계 — 분할점을 코스 격자 정렬로 강제(파인은 자동 정렬).
  farfield40 Ny: L0 240/L1 193(!)... **홀수 Ny 레벨 존재** → 균등분할 대신 '코스-정렬 최근접' 분할 규칙 필요.
...
- (e) sponge/BC의 y-face: y-분할이므로 ymin/ymax BC는 끝 랭크만 적용 — bc_manager 슬라이스화.
