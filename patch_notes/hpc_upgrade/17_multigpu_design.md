...
### ✅✅✅ 코드 트랙 공식 마감 (2026-07-13)
- 단일 확정 1.084 s/step(NR=1 --dist-init) → **동코드 분해이득 2.45×, 병렬구간 효율 ~100%**
  (kernel 0.417→0.103×4, coupling 0.395→0.096×4). 잔여=BEM 복제+스큐(구조적 상수, 문서화).
- 구코드 대비 총 진보: 단일 2.9×, 4-rank 7.0×. 풀런 3.9h. 용량 ~450-500M(--dist-init).
- bit-사다리가 잡은 실결함 총 9건(부록 E 확정 목록). 다음=물리 트랙(kernel novelty β).

### 최종 실런 시험: 단일(GPU0) vs 3-rank(GPU1-3), 2 rev 동시 실행 (2026-07-13)
- NR=3 신규 구성(axis=y, bounds[0,112,129,240], worst 0.348=효율 96%) — balance_cuts 일반성 실증.
- **판독: 전 항목 완전 bit-identical** — 마커 positions/alpha/CL/F_n/F_θ, thrust(ΣF_n 8자리),
  **필드 L0-L4 전부 max|Δu|=0** @1257·@2514(=L4 40,224 substeps).
- 사전예측("힘 lastbit 시드→카오스 성장")은 좋은 방향으로 반증 — 기제: ALM 부분합의 f64 재결합
  차(~1e-16)가 **f32 캐스트 반올림 방화벽**에서 소멸(경계 걸릴 확률 ~1e-9/값; 이 런 ~3e7회 반올림
