...
def main():
    ...
    # Taylor-Green IC -> f0 = f_eq(rho, u) via the solver's own equilibrium.
    Nx, Ny, Nz = SHAPE
    ...
    uz = cp.zeros_like(ux)
    rho0 = cp.ones(SHAPE, dtype=cp.float32)
    u0 = cp.stack([ux, uy, uz])
    f0 = coll.compute_equilibrium(rho0, u0).astype(cp.float32)

    sim.set_distribution(f0)
    if not sim._use_esoteric:
        print("  RESULT: FAIL (esoteric path not enabled by LBM_ESOTERIC=1)")
        sys.exit(1)

    # sim.f is now in Esoteric layout; total sum still equals total mass.
    ...
    umax = float(cp.max(cp.abs(sim.u)))
    drift = (m1 - m0) / m0

    print(f"[eso_sim_integration_gate] {SHAPE}, {NSTEPS} steps, BGK periodic")
    print(f"  _use_esoteric = {sim._use_esoteric}   f_post = {sim.f_post}")
    print(f"  mass drift = {drift:+.3e}   |u|max = {umax:.4f}   NaN = {nan}")
    ok = (sim._use_esoteric and not nan and abs(drift) < 1e-5
          and sim.f_post is None and sim._esoteric_step == NSTEPS)
