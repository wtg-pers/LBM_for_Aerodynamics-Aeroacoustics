...
| 단일(NR=1 러너, --dist-init) | **1.084 s/step** (kernel 0.417 / coupling 0.395 / alm 0.193 / halo-self 0.067) |
...
### 부록 E 확정 (2026-07-13) — 코드 트랙 마감

| 지표 | 값 | 비고 |
|---|---|---|
| 동코드 분해이득 (러너 1-rank 1.084 → 4-rank 0.442) | **2.45×** | 동일 코드·동일 측정경로 |
| 병렬화 구간(kernel+coupling)의 분할 효율 | **~99-103%** | 0.417→0.103×4, 0.395→0.096×4 |
| 잔여 비병렬 | BEM 복제 ~0.08 + lockstep 스큐 ~0.1 | 문서화된 구조적 상수 |
| 구코드 단일(3.1) 대비 총 진보 | 단일 **2.9×**(→1.08) · 4-rank **7.0×**(→0.442) | 백로그 #3-#5가 단일에도 기여 |
| 4-GPU 용량 | ~110M → **~450-500M 셀** (--dist-init) | 23GB 하드리밋 검증법 확립 |

검증 체계 총결산: 전 과정에서 bit-parity 사다리가 잡아낸 실결함 = esoteric init 롤 누락,
SGS ghost 깊이, x-체인 퇴화, 균등분할 infeasibility, shape-의존 라이브러리 리덕션(§6),
연산자 미공유(f2c 인라인), verify 진입점 파손, 재시작 로터 상태(production 버그),
replicated NR=1 메모리 이중보유 — **9건**. tolerance-only 검증이었다면 이 중 §6·연산자
미공유·재시작 로터는 조용히 물리를 오염시켰을 것이다.
