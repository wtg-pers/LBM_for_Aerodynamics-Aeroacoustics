"""
APC 9x4.5MR Propeller -- Axial Flight J=0.129, Cumulant + ALM + MLG (5-Level)

UIUC blade geometry + dual airfoil (E63 + NACA4412).
Target (propeller convention): CT, CP from UIUC database.

Grid (D=40 on L0, refine 2x per level -> L4 has 640 cells/D, dx_L4=0.357 mm):
  ...
  - L3: ±0.1D up / 0.2D down / ±0.6D lat  (320 c/D)
  - L4: hub ±0.025D axial / ±0.525D lat   (640 c/D, rotor disk + Gaussian eps)

Tip resolution at L4: chord_tip=5.6mm = 15.7 L4 cells, eps_tip=3.92 L4 cells.

Total ~52.86M cells (~12 GB distribution, ~24 GB with buffers).

Sponge: thickness=20 lu (=0.5D in L0), strength=0.1 (mild absorption).

Usage:
    python main.py --config configs/alm/alm_9x45e_J0129_mlg5_test.py

NOTE: This is a positive-CT isolation test (Step 1 in handoff.md).
J=0.129 lies well inside the propeller (thrust-producing) operating regime;
UIUC measurement gives CT > 0. If this run also yields CT < 0, the regression
is in the algorithm/coordinate convention, not in the J=0.608 windmilling
boundary behavior.
"""
...
print(f"  [APC 9x4.5MR Axial J={J}, RPM={RPM}, MLG 5-Level]")
...
# L4: hub +/-0.025D axial x +/-0.5D lateral  (640 cells/D)
# Axial 0.025D = 1 L0 cell each side -> 2 L0 cells total -> 32 L4 cells, hub
# at the L4 midpoint plane.
# Lateral 1.0D total (each side 0.5D = disk radius) reduced from 1.05D to
# tighten memory. ε_tip cutoff (~0.006D) at the tip marker spills <1% of the
# Gaussian outside box; marker at r/R=0.99 (= 0.495D) still has its full
# spread well inside box.
L4_X_MIN = HUB_X - 1                                # 55
L4_X_MAX = HUB_X + 1                                # 57
L4_Y_MIN = HUB_Y - int(0.5 * D)                     # 100-20 = 80
L4_Y_MAX = HUB_Y + int(0.5 * D)                     # 120
L4_Z_MIN = HUB_Z - int(0.5 * D)                     # 80
L4_Z_MAX = HUB_Z + int(0.5 * D)                     # 120
...
# SGS off (mlg5 OOM mitigation: WALE pre-pass buffers consume extra GBs at
# the fine levels; ALM force is BEM-polar driven and not directly SGS-sensitive).
...
_folder = f"result_apc9x45e_J{J:.3f}_Re{RE // 1000}k_mlg5"
