"""
APC 9x4.5MR Propeller -- Hovering @ 6043 RPM, Cumulant + ALM + MLG (3-Level)

UIUC blade geometry + dual airfoil (E63 + NACA4412).
Hovering (J = 0) — Figure of Merit / C_T / C_P metrics reported in
rotorcraft convention (see Rotor.compute_coefficients with mode='auto').
...
Usage:
    python main.py --config configs/alm_9x45e_hovering_6043rpm_mlg3.py
"""
...
#   omega_lu = u_max / R_lu  →  steps_per_rev = 2π·R_lu / u_max = π·D / u_max
...
print(f"  [APC 9x4.5MR Hover RPM={RPM}, MLG 3-Level]")
print(f"  Re_75={RE:,}, D={D}, {N_BLADES} blades")
print(f"  tip_speed={TIP_SPEED:.2f} m/s, u_max_lu={U_MAX_LU}")
...
# =============================================================================
# S6. CONFIG BLOCKS (new format: physics / grid / numerics separated)
# =============================================================================
simulation = {
    "device_mode":     "gpu",
    "precision":       "float32",
...
physics = {
    "rho":            RHO_PHYS,
    "U_inf":          U_INF_PHYS,       # 0 m/s (hovering)
    "Re":             RE,
    "L_char":         D_PHYS,
...
# Hovering: no imposed freestream — far-field open, slipstream absorbed
# at xmax via sponge. eq/neumann/sponge only (no regularized_*).
boundaries = {
...
actuator_line = {
    "enabled": True,
    "rotor": {
        # rpm + radius are read by UnitConverter (tip_speed = ω·R → dt_phys).
        "rpm":              RPM,
        "radius":           R_PHYS,
...
# Suitable for ALM rotor wake LES — auto-damps near solid surfaces (S^d→0).
...
_folder = f"result_apc9x45e_hovering_{RPM}rpm_Re{RE // 1000}k_mlg3"
...
if __name__ == "__main__":
    ...
    # nu_lu placeholder for the scaler (only used for level τ scaling);
    # real value is derived by UnitConverter at runtime.
