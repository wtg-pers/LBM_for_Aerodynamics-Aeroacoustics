"""LE-resolution ladder point c=170 (patch 79) — thin-span (nz4 lineage).
...
Run (ONE GPU each; two points can run in parallel on two cards):
    LBM_ESOTERIC=1 python main.py \
        --config configs/stl/naca0012_a10_surfel_re1e6_nz4_c170.py --gpu 0
"""
...
# nz_frac chosen so Nz=6 (even; round(0.04*170)=7 would be the first
# odd-Nz case ever run — not a risk worth bundling into a discriminator)
config = _m._build(170, wall_bc="surfel", nz_frac=0.0353)
...
_folder = "results_naca0012_a10_surfel_re1e6_nz4_c170"
config["output"] = dict(
    config["output"],
