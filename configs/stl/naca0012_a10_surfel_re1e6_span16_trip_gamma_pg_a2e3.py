"""A-sweep arm amp=2e-3 (patch 66 pre-registration, run on the
gamma+PG base — patch 81 §8 verdict).

ONE knob over naca0012_a10_surfel_re1e6_span16_trip_gamma_pg:
trip_forcing.amp_lu = 2e-3 (base 1e-3). Discriminator: the fore
...
Run (cluster, 2x24 GiB) — --mca pml ucx REQUIRED:
    ...
    LBM_ESOTERIC=1 mpirun --mca pml ucx -n 2 python main.py --mpi \
        --config configs/stl/naca0012_a10_surfel_re1e6_span16_trip_gamma_pg_a2e3.py \
...
_folder = "results_naca0012_a10_surfel_re1e6_span16_trip_gamma_pg_a2e3"
config["output"] = dict(config["output"],
                        output_dir=f"./{_folder}/vtk",
                        checkpoint_dir=f"./{_folder}/checkpoints",
                        csv_dir=f"./{_folder}/csv")
config["trip_forcing"] = dict(config["trip_forcing"], amp_lu=2e-3)
