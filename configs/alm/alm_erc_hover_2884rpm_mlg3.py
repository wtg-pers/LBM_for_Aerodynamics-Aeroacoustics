"""
ERC Propeller -- Hover @ 3923 RPM, Cumulant + ALM + MLG (3-Level) + WALE SGS
...
Polars: 87 CSV files in input_files/erc_prop/{14..100}section.csv
        (each holds 3-Re x 51-AoA polar from BEMT generation).

Coordinate convention: thrust along -x (rotor disk faces +x freestream),
which matches the existing APC ALM configs. Hover -> U_inf = 0.

Usage (run from project root that contains main.py, configs/, input_files/):
    python main.py --config configs/alm/alm_erc_hover_3923rpm_mlg3.py
"""
...
U_INF_PHYS   = 0.0                                   # [m/s] hover

# Reference chord/Re at r/R = 0.75 (interval midpoint i=61, chord=2.74e-2 m)
...
print(f"  [ERC prop Hover RPM={RPM}, MLG 3-Level, WALE]")
...
# Hover: open far-field via eq, sponge at xmax for slipstream.
...
_folder = f"result_erc_hover_{RPM}rpm_mlg3"
output = {
