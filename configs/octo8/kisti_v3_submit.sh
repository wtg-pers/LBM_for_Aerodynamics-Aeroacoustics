#!/bin/bash
# ============================================================================
# octo8 v3 — KISTI SLURM 제출 스크립트 (자동 재제출 포함)
# ============================================================================
...
#          --comment=inhouse configs/octo8/kisti_v3_submit.sh
#
...
#   scancel <jobid>                                  # 연쇄 중단
#   touch STOP_OCTO8_V3                              # 다음 턴에 우아하게 종료
#
...
#  ① 여기만 채우면 된다 (클러스터 고유값)
# ─────────────────────────────────────────────────────────────
# 파티션 (전부 wall-clock 48h). v3 = 178.1 M 셀 / 100 rev = 3.59e13 업데이트(v2 대비 -11%),
# LBM 은 대역폭 지배라 총 대역폭으로 환산한 추정:
#     cas_v100_2     V100 x2   1.8 TB/s   11.6 GB/rank   1.4~4.6 일  (재제출 필수)
#     cas_v100nv_8   V100 x8   7.2 TB/s    2.9 GB/rank    9~28 h
#   ★ amd_a100nv_8   A100 x8  ~16 TB/s    2.9 GB/rank   3.6~13 h   <- 한 잡에 끝남
#     amd_h200nv_8   H200 x8  ~38 TB/s    2.9 GB/rank   1.6~5.3 h
...
# 복제 빌드(전 도메인 필드를 랭크마다)는 v3 178.1 M 셀 x ~205 B = **랭크당
# ~36 GB**. A100 80GB 면 들어가므로 DIST_INIT="" 로 두면 conservation 진단이
# 살아난다. 40GB 판이면 --dist-init 이 필수.
# 대화형 잡에서 nvidia-smi 로 실제 용량을 확인한 뒤 결정할 것.
#   ※ BC/기하 빌드 피크는 별개이고 이미 0.68 GB 다(ibb 링크 희소화, 567527d).
#      --dist-init 이 없애는 것은 필드 쪽 ~36 GB 이다.
DIST_INIT="--dist-init"

# ─────────────────────────────────────────────────────────────
#  ② SLURM 지시자 — ①의 값이 여기 반영되도록 재제출 시 --export 로 넘긴다
# ─────────────────────────────────────────────────────────────
#SBATCH --job-name=octo8_v3
#SBATCH --nodes=1
#SBATCH --ntasks=8
#SBATCH --cpus-per-task=4
#SBATCH --gres=gpu:8
#SBATCH --output=logs/octo8_v3_%j.out
#SBATCH --error=logs/octo8_v3_%j.out
...
set -u
cd "$REPO_DIR"
CFG="configs/octo8/octo8_v3_hover.py"
STOPFILE="STOP_OCTO8_V3"
...
# ─────────────────────────────────────────────────────────────
CKPT_DIR="./result_octo8_v3/checkpoints"
RESTART=""
...
DONE=$(python - "$CFG" "$CKPT_DIR" <<'PYEOF'
...
fs = glob.glob(os.path.join(sys.argv[2], "*.npz"))
