...
GRID_PRESETS = {
    ...
    "farfield40": (40,
                   [(0.65625, 1.84375, 1.15625),
                    ...
                    (0.03125, 0.0625,  0.53125)],
                   {"nx_D": 6.5, "ny_D": 6.0, "nz_D": 6.0, "hub_x_D": 1.5}),
    # ★farfield40_l4w03/04 (2026-07-21): farfield40에서 **L4 슬랩만 wake로 확장**.
    #   근거: shadowgraphy 궤적 검증([[project_shadowgraphy_wake_validation]]) —
    #   팁와류 코어가 L4→L3 경계(z/R −0.11, age~130°)에서 core radius 2배 점프
    #   = 격자 소산 지배. L4를 하류로 늘려 코어가 L4에 머무는 age를 연장, wake
    #   궤적·코어 소산이 회복되는지 A/B (s1 iso/iso 기준, 다른 것 전부 동결).
    #   - l4w03: L4 down 0.0625→0.15625D(6.25lu, z/R −0.3125, age~300° L4 유지).
    #     L3 불변 — L4·L3 axial margin 0.09375D ≥ 0.08D tip 가이드 충족.
    #   - l4w04: L4 down 0.20625D(8.25lu, z/R −0.4125) + L3 down 0.25→0.3125D
    #     (margin 0.10625D 유지; L3-L2 margin 0.6875D로 여유).
    #   셀(farfield40 91.6M 실측에서 스케일): l4w03 L4 117×681²=54.3M, 총 119.4M
    #   (updates ×1.63) / l4w04 L4 149×681²=69.1M + L3 149×441²=29.0M, 총 138.1M
    #   (updates ×1.99). esoteric 실측 207B/cell → 2-rank당 12.4 / 14.3GB = 4090 OK.
    "farfield40_l4w03": (40,
                         [(0.65625, 1.84375, 1.15625),
                          (0.4,     1.0,     0.84375),
                          (0.125,   0.25,    0.6875),
                          (0.03125, 0.15625, 0.53125)],
                         {"nx_D": 6.5, "ny_D": 6.0, "nz_D": 6.0, "hub_x_D": 1.5}),
    "farfield40_l4w04": (40,
                         [(0.65625, 1.84375, 1.15625),
                          (0.4,     1.0,     0.84375),
                          (0.125,   0.3125,  0.6875),
                          (0.03125, 0.20625, 0.53125)],
                         {"nx_D": 6.5, "ny_D": 6.0, "nz_D": 6.0, "hub_x_D": 1.5}),
    # ★farfield40_mlg4 (2026-07-22): farfield40에서 **L4 슬랩 제거** — ALM이
    #   L3(dx=L0/8)에 직접 앉음. 목적: 근접 wake(z/R +0.275~−0.525)를 단일
    #   레벨이 커버 → 레벨 인터페이스가 코어 소산·하강에 미치는 영향을
    #   해상도 효과와 분리(l4wake A/B의 상보 실험: l4wake=경계 이동, 이건
    #   경계 제거). ε=0.25c(chord 기반)라 물리 ε은 mlg5와 동일; ε/Δx는
    #   inboard 3.3 / tip 2.02(2Δx floor 한계선상 합법). n64 시 δr=1.84
    #   L3셀 < ε ✓. 셀: L0 15.0 + L1 7.7 + L2 17.4 + L3 25.1 = 65.1M
    #   (updates/step 0.42× vs mlg5) → 4-rank ~3.4GB/GPU.
    #   ⚠하중은 s1과 동일하지 않을 것(D-sweep에서 fine dx가 CT에 유의:
    #   coarser→CT↑ 경향) — 비교 축은 wake 지표가 1차, 하중은 참고.
    "farfield40_mlg4": (40,
                        [(0.65625, 1.84375, 1.15625),
                         (0.4,     1.0,     0.84375),
                         (0.125,   0.25,    0.6875)],
