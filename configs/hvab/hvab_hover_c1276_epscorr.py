"""HVAB hover, collective 12.76 deg, M_tip=0.65 — viscous-core smearing 보정 ON.

A/B 쌍: hvab_hover_c1276.py (보정 OFF + Prandtl ON, 기존 baseline) vs 이 파일.
...
경험적 Prandtl과 중복 방지). 목적: CT/CP 팁 과예측 완화 → FM peak 회복 여부 확인.
    python main.py --config configs/hvab/hvab_hover_c1276_epscorr.py
"""
import os, sys
sys.path.insert(0, os.path.dirname(__file__))
from _hvab_hover_base import build_config
config = build_config(
    collective_deg=12.76, mtip=0.65,
