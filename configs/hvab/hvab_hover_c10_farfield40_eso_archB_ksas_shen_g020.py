"""HVAB hover c10 M0.65 — archB (era-B best FM) + Shen g=0.2 tip loss.
...
4-GPU run:
    mpirun --mca pml ucx -x LBM_ESOTERIC=1 -n 4 python main_mpi.py \
        --config configs/hvab/hvab_hover_c10_farfield40_eso_archB_ksas_shen_g020.py \
        --steps 31425 --log-every 64 --cuda-aware 1 --dist-init \
        --devices 0,1,2,3 --vtk-every 1257 --vtk-fields-last 5 \
        --ckpt-every 31425 --csv archB_shen_g020.csv
"""
import os, sys
...
config = build_config(
    ...
    prandtl_loss={"enabled": True, "model": "shen", "g": 0.2,
    ...
    run_tag="farfield40_eso_archB_ksas_shen_g020",
)
