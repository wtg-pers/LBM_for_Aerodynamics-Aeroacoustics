"""
3D Cylinder Re=3900 — IBB + padded MLG-3, D=20  (v2: wake-extended)

v2 differences vs cyl_Re3900_ibb_mlg3_3d.py:
    - L1_DOWN_D : 4D → 10D     (wake mid-range covered at D=40)
    - L2_DOWN_D : 2D → 4D      (recirculation + early shedding at D=80)
    - Sponge thickness: 30 → 50 lu  (1.5D → 2.5D — softer outflow ramp)

Motivation: v1 fields look under-resolved past x≈+4D and the L0 sponge
(thickness=30 lu = 1.5D) sits relatively close to the wake. At Re=3900
the recirculation length L_w/D ≈ 1.20 but coherent shedding eddies
persist 8–10D downstream. Pushing L1 to +10D and L2 to +4D resolves
the early/mid wake at fine grid; thicker sponge reduces reflection.
...
Domain matched to Rajani et al. 2016 Run 6/7 (3D LES reference):
    ...
    L1     : D=40  ±2D up / +10D down / ±2D lat   (wake mid-range)
    L2     : D=80  ±1D up / +4D down  / ±1D lat   (recirculation;
                                                   cylinder + 0.5D padding)

Estimated cells: ~48M; D3Q27 cumulant single GPU memory ~12 GB.
...
L1_UP_D    = 2.0
L1_DOWN_D  = 10.0       # v2: extended from 4D for wake mid-range
L1_LAT_D   = 2.0

L2_UP_D    = 1.0
L2_DOWN_D  = 4.0        # v2: extended from 2D for full recirculation
L2_LAT_D   = 1.0
...
print(f"  [cyl Re={RE:.0f}, 3D MLG-{NUM_LEVELS} v2 (D_fine={CPC_FINEST}), D={D_LU} L0, {WALL_BC}]")
...
boundaries = {
    ...
    "outlet": {"location": "xmax", "method": "sponge",
               ...
               "thickness": 50, "strength": 0.5},   # v2: 30 → 50 lu (2.5D)
...
_folder = f"result_cyl_Re3900_ibb_mlg3_3d_v2"
output = {
