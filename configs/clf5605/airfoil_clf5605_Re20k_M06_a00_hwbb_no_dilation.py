"""
CLF5605 Airfoil — 2D D2Q9 Cumulant + 4-level MLG, α=+0°, HWBB wall

HWBB baseline variant of the α=0 MLG case. Pair with the IBB sibling
`airfoil_clf5605_Re20k_M06_a00_ibb.py` to compare half-way
bounce-back vs. Bouzidi interpolated bounce-back. Output directories
carry a `_ibb` / `_hwbb` suffix so both runs coexist on disk.

Usage:
    python main.py --config configs/airfoil_clf5605_Re20k_M06_a00_hwbb.py
"""
...
WALL_BC = "hwbb"       # this file. Sibling _ibb.py sets "ibb".
...
internal_geometry = {
    "airfoil": {
        ...
        "wall_bc": WALL_BC,
        # Outward polyline dilation [lattice units of EACH level]. 0.6 lu
        # outward at every level closes any sub-pixel TE gaps and blocks
        # the diagonal lattice-link leakage.  Same value as the IBB sibling
        # uses, so the airfoil GEOMETRY seen by both BCs is identical;
        # IBB-vs-HWBB comparison then isolates the wall-BC algorithm
        # contribution to Cd / Cl rather than confounding it with mask
...
_folder = (
    f"no_dilation_result_clf5605_MLG_Re20k_M06_a"
