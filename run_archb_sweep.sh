...
run_case () {
    ...
    $MPIRUN -n 2 python main_mpi.py \
        ...
        --vtk-every "$VTKEVERY" --csv "$csv" 2>&1 | tee "sweep_archb_c${deg}.log"
...
FAILED=""
run_case 6  configs/hvab/hvab_hover_c6_farfield40_eso_archB_mpi2.py  sweep_archb_c6.csv  || FAILED="$FAILED 6"
run_case 8  configs/hvab/hvab_hover_c8_farfield40_eso_archB_mpi2.py  sweep_archb_c8.csv  || FAILED="$FAILED 8"
run_case 10 configs/hvab/hvab_hover_c10_farfield40_eso_archB_mpi2.py sweep_archb_c10.csv || FAILED="$FAILED 10"
run_case 12 configs/hvab/hvab_hover_c12_farfield40_eso_archB_mpi2.py sweep_archb_c12.csv || FAILED="$FAILED 12"

echo "[sweep] post-processing (failed cases:${FAILED:- none})"
python src/utilities/postprocess_collective_sweep.py --suite archb --out sweep_analysis_archb
