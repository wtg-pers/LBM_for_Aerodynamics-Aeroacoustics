    ...
    def _checkpoint_payload(self, sim, include_extra: bool = True):
        ...
        if include_extra:
            ...
            for uid in range(len(r.blocks)):
                ...
                if not wm:
                    continue
                strips = {}
                if r.owns[uid] and r.parts[uid].own_count > 0:
                    pu = r.parts[uid]
                    lay_l, _ = eso_wall_mail_layout(
                        wm, tuple(pu.local_shape))
                    own_sl = slice(pu.ghost, pu.ghost + pu.own_count)
                    for bit, (off, area, (l1, l2), axes) in lay_l.items():
                        seg = r.lv[uid].wall_mail[off:off + 9 * area] \
                            .reshape(9, l1, l2)
                        sl = [slice(None)] * 3
                        sl[1 + axes.index(r.axis)] = own_sl
                        strips[bit] = cp.asnumpy(seg[tuple(sl)])
                wall_strips[uid] = comm.gather(strips, root=0) \
                    if comm is not None else [strips]
        ...
        if include_extra:
            ...
            if wall_strips:
                import numpy as _np
                from src.kernels.esoteric_d3q27 import eso_wall_mail_layout
                for uid, per_rank in wall_strips.items():
                    b = r.blocks[uid]
                    wm = r.wall_masks[uid]
                    pu = r.parts[uid]
                    g_dims = list(pu.local_shape)
                    g_dims[r.axis] = sum(c for _s, c in r.range_table[uid])
                    lay_g, tot_g = eso_wall_mail_layout(wm, tuple(g_dims))
                    gm = _np.zeros(tot_g, _np.float32)
                    for rk, st in enumerate(per_rank):
                        s0, cnt = r.range_table[uid][rk]
                        if cnt == 0 or not st:
                            continue
                        for bit, arr in st.items():
                            ...
                            seg[tuple(sl)] = arr
                    _sfx = "" if per[b.level] <= 1 else f"_b{b.index}"
                    _kb = f"L{b.level}{_sfx}" if b.level > 0 else "L0"
                    extra[f"wall_mask_{_kb}"] = wm
                    extra[f"wall_mail_{_kb}"] = gm
