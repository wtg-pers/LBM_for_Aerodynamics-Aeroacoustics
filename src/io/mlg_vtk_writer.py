...
class MLGVTKWriter:
    ...
    def __init__(
        ...
        # False = this run will not WRITE volume VTK (--no-vtk, non-io MPI
        # rank): the writer is metadata-only, so an existing series in the
        # target dir must not block the run on a units mismatch.
